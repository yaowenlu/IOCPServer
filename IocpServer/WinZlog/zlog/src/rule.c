...
#ifdef _MSC_VER
int stdwrite(int fd,char *data,int len) {
	int numb = 0;
	int status = WriteFile(fd,data,len,&numb,NULL);
	if(status == 0) return(-1);
	return(numb);
}
#else
#define stdwrite(f,s,l) write(f,s,l)
#endif
